...
- 浏览器后退/前进在应用内导航：进影片详情、切 Tab、翻页、搜索、筛选都会记入历史，按「返回」回到上一个视图而不是离开站点；耗尽应用内历史后再按返回才离开
- 启动时若有多个板块存在断点，会依次自动续抓（电影跑完接着跑电视剧）；手动停止其中一个板块后，剩余板块不再自动启动

## 部署注意

- **必须单 worker 运行**（默认即如此）：应用启动时会拉起后台线程做断点续抓、每日定时增量与详情拉取，给 uvicorn 加 `--workers N` 会得到 N 份互相冲突的后台任务并重复请求站点
